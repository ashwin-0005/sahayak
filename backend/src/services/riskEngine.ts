// Risk engine — PURE, DETERMINISTIC, NO I/O, NO AI.
//
// NOTE: thresholds below are DEMO values for a hackathon. They MUST be
// validated against current national guidelines (e.g. NHM / WHO HEARTS /
// PMSMA for pregnancy) before any real clinical use.
//
// Design principle: CONSERVATIVE. Missing / contradictory / uncertain data
// escalates to the SAFER option, never de-escalates. All rules are evaluated
// and the HIGHEST severity wins, with every matching reason code returned.
...
export interface RiskResult {
  level: RiskLevel;
  reasonCodes: string[];
  adviceKey: "refer_urgent" | "visit_clinic_week" | "continue_home_care";
...
export function assessRisk(input: RiskInput): RiskResult {
  ...
  if (condition === "pregnancy") {
    ...
    if (!hasSys && !hasDia) {
      ...
      // For non-hypertension (diabetes/tb), missing BP alone is not flagged
      // unless other rules fire — conservative but avoids noise.
    } else {
  ...
  if (condition === "diabetes") {
    ...
    // Non-diabetic but sugar supplied: still flag extremes conservatively.
    const v = sugarMgDl as number;
    if (v < 70) escalate(2, "HYPOGLYCEMIA");
    else if (v >= 300) escalate(2, "SUGAR_VERY_HIGH");
  }
